...
var Model = {
    ...
    UrlToRepo: function (repo, fileName, filePath, line, rev) {
        return UrlToRepo(this.repos[repo], filePath || fileName, line, rev);
    },
            ...
            statsView = (
                <div className="stats">
                    <div className="stats-left">
                        <a href="excluded_files.html" className="link-gray">
                            Excluded Files
                        </a>
                    </div>
...
var FileContentView = React.createClass({
    ...
    getFileUrl: function (repo, fileName, filePath, rev) {
        const fileUrl = Model.UrlToRepo(repo, fileName, filePath, null, rev);
        return fileUrl;
    },
    render: function () {
        ...
        var matches = blocks.map(function (block) {
            ...
            return <div className="match">{lines}</div>;
        });
        ...
        return (
            <div className={"file " + (this.state.open ? "open" : "closed")}>
                <div className="title" onClick={this.toggleContent}>
                    <a
                        href={this.getFileUrl(repo, fileName, filePath, rev)}
                        target="_blank"
                        rel="noopener noreferrer"
                    >
                        {fileName}
                ...
                <div className="file-body">{matches}</div>
            </div>
        );
    },
});
...
var FilesView = React.createClass({
    ...
    render: function () {
        ...
        if (matches.length < totalMatches) {
            more = (
                <button className="moar" onClick={this.onLoadMore}>
...
var App = React.createClass({
    ...
    updateHistory: function (params) {
        var path =
            ...
            "&literal=" +
            encodeURIComponent(params.literal) +
            "&files=" +
            encodeURIComponent(params.files) +
            "&excludeFiles=" +
            encodeURIComponent(params.excludeFiles) +
            "&repos=" +
            params.repos;
